...
export default function OneClickApp() {
  ...
  const safeRendered = rendered ?? <div className="min-h-screen bg-[#F7F9FC] px-4 py-16"><div className="mx-auto max-w-xl rounded-[28px] border border-gray-200 bg-white p-6 shadow-sm"><div className="text-xs font-black uppercase tracking-[.16em] text-[#FF5C5C]">Preview mismatch</div><h1 className="mt-3 text-2xl font-black">Screen unavailable in this preview</h1><p className="mt-3 text-sm leading-6 text-gray-500">This preview was built from an incomplete screen set. Go back to the previous step or open the latest deployment.</p><button onClick={() => go(previousScreen(screen))} className="mt-6 min-h-11 w-full rounded-2xl bg-[#0B1D33] px-4 font-extrabold text-white">Back to previous screen</button></div></div>;

  return <div className="relative min-h-screen font-sans text-[#1E1E1E]">{safeRendered}<button onClick={() => setNavigatorOpen(true)} aria-label="Open prototype navigator" className="fixed bottom-[78px] right-3 z-50 flex min-h-11 items-center gap-2 rounded-full border border-gray-200 bg-white px-3 py-2 text-xs font-extrabold shadow-lg lg:bottom-4 lg:right-4"><span className="grid h-6 w-6 place-items-center rounded-full bg-[#0B1D33] text-[9px] text-white">{meta.id.slice(1)}</span><span className="hidden sm:inline">Review screens</span><ChevronDown size={14}/></button>{navigatorOpen && <PrototypeNavigator current={screen} go={go} close={() => setNavigatorOpen(false)}/>}</div>;
}
